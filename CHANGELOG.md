...
- Added Initial Dungeon Layout V1: the deterministic starting map now drops from 70 dug cells (~19.0%) to 40 dug cells (~10.9%), keeps ~89.1% rock, removes the three free large rooms, and preserves a valid but narrower entry -> treasure -> boss route. Smoke tests now guard density, rock majority, safe-zone compatibility, route validity, and non-trivial path length.
