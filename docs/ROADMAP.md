...
```
M0 Prototype ──▶ M1 Gameplay Foundation ──▶ M2 Living Kingdom ──▶ M3 Content Expansion ──▶ M4 Art Direction ──▶ M5 Polish ──▶ M6 Release Candidate
   (acquis)         (base solide)             (le royaume vit)      (plus de contenu)        (identité visuelle)   (finition)     (prêt à sortir)
```
...
- Salles et portes distinctes (remplacent le carvage 3×3 actuel).
- Upgrades de pièges entre les vagues.
...
- Un joueur peut construire une salle spécialisée et la porte associée sans régression sur la validation de chemin.
...
**Objectif** : combler l'écart principal identifié dans le GDD — faire vivre la guilde et le royaume au-delà de champs de données statiques.
...
- Alarme du royaume (`alarm`) branchée sur la fréquence/composition des vagues.
- Pressions régionales (peur, cupidité, gloire, prime) influençant les rumeurs et les plans d'expédition.
- Apprentissage collectif étendu : cartes de pièges partagées, mémoire de chemin collective.
- Ciblage vindicatif par individu nommé plutôt que par type de défense.
- Écran de taverne dédié avec rumeurs concurrentes.

**Critères de validation** :
- Un joueur peut observer un changement de comportement du royaume directement attribuable à l'alarme ou à une pression régionale, distinct de la simple pression de rôle existante.
...
**Objectif** : élargir le vocabulaire de jeu (contenu) une fois les fondations et le royaume vivant en place.
...
**Fonctionnalités prévues** :
- Passe d'équilibrage complète sur l'économie, le scaling de vague et les capacités de boss.
...
- Aucune chute de performance mesurable jusqu'à un seuil de vague défini par les tests de charge.
