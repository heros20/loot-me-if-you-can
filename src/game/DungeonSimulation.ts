...
const CONSTRUCTION_TOOLS: Array<{
  ...
  },
  {
    type: 'reseal',
    name: 'Reboucher',
    description: `Rebouche une case creusee non critique sans bloquer le chemin obligatoire. ${RESEAL_TILE_COST} or.`,
    category: 'construction',
...
export class DungeonSimulation {
  ...
  previewResealTile(cell: GridCell) {
    ...
    if (!validation.valid) {
      return {
        ...
        cost: 0,
        changed: 0,
        message: validation.reason ?? 'Rebouchage refuse: cette roche condamnerait une route obligatoire.',
      };
    }
  ...
  private buildAdaptiveRoster(wave: number): AdventurerRole[] {
    ...
    const memory = {
      ...this.buildMemoryWithKingdomPressure(wave),
      rolePressure: {
        ...this.buildRolePressureWithKingdomMemory(wave),
  ...
  private damageAdventurer(
    ...
    if (actualDamage > 0) {
      ...
      this.state.runtime.stats.heavyDamageCells ??= {};
...
function pushUniqueCell(cells: GridCell[] | undefined, cell: GridCell): void {
  if (!cells) {
    return;
  }
...
function pushUniqueSpecialTreasure(
  observations: Array<{ kind: DungeonTreasureKind; cell: GridCell }> | undefined,
  kind: DungeonTreasureKind,
  cell: GridCell | null,
): void {
  if (!observations || !cell || !isSpecialTreasureKind(kind)) {
    return;
  }
