...
- Dungeon Structure V2 (see `docs/DUNGEON_STRUCTURE_V2.md`, `docs/DUNGEON_ANCHORS_V1.md`, and `docs/INITIAL_DUNGEON_LAYOUT_V1.md`):
  ...
  - V1.3 Rebouchage / Murs V1 is implemented: the player can reboucher a dug floor/room tile for 2 gold, but not through entities, doors, treasures, boss/entry anchors, or mandatory path validation.
...
- Human Adventurer Behavior V0 is implemented: tank-first boss engagement lock, opportunistic loot on/near path, short room evaluation, group lead limits, backline hold, rogue flank/wait behavior, and visible special-item impact. Future work: tune delays/weights after manual runs; do not expand this into cartography, inventory, sub-bosses, or deeper Kingdom AI yet.
- Hopital / Repos / Refus V1 is implemented: survivors can be injured, resting, or shaken for one expedition, stay alive and persistent, keep special treasure bonuses, and are skipped by composition until recovered. Future work: tune thresholds after manual runs; do not expand this into paid care or a hospital screen yet.
- Kingdom Remembers V1 is implemented: the kingdom/guild now stores imperfect in-run facts reported by survivors only, with confidence, decay, stale route handling, light composition/behavior pressure, and sidebar/tavern rumors. Future work: tune confidence thresholds after manual runs; do not expand this V1 into full cartography, Cartographer, guild rosters, or trans-run memory yet.
...
- Survivor Chronicle follow-up: tune the exact narrative line pool after playtests, and later decide how much of it should feed deeper Kingdom Memory without adding cartography in this V1.
...
- Expand Kingdom Remembers beyond V1 into partial cartography only after the survivor-fact model is tuned.
...
- Rebouchage / Murs V1: the Mur placeholder is now the Reboucher build tool, with a 2 gold cost, strict refusal on occupied/critical cells, simulated topology validation, updated tile/pathfinding blockers, and smoke coverage for success, refusals, and cost behavior.
- Hopital / Repos / Refus V1: survivor profiles now track short recovery states, composition ignores unavailable survivors and fills with volunteers, mandatory roles still win, and tavern/sidebar/report distinguish injured/resting/shaken absences from deaths and tactical reserve.
